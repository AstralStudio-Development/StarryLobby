...
public class BedWarsNPC extends AbstractNPC {
    ...
    public List<String> getNpcDisplayName(Player player) {
        ...
        switch (languageType) {
            default:
                lines.add("&d&l版本更新 & 新任务");
                lines.add("&b起床战争");
                lines.add(PlaceholderAPI.setPlaceholders(player, "&e当前有" + Main.getInstance().getConfig().getString("total.BedWars") + "名玩家正在游玩"));
                break;
        }
        return lines;
    }
    ...
    public Location getNpcSpawnLocation() {
        return new Location(Bukkit.getWorld("world"),-25.5,93.0,2.5,95,0);
    }
    ...
    public ItemStack getNpcHeldItem() {
        return new ItemBuilder(Material.BED).build();
    }
