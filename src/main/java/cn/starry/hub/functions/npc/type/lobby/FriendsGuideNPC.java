 ...
 */
public class FriendsGuideNPC extends AbstractNPC {

    private HashMap<Player,Integer> round = new HashMap<>();

    @Override
    public String getNpcInternalName() {
        return "friends_guide";
    }
    ...
    public Location getNpcSpawnLocation() {
        return new Location(Bukkit.getWorld("world"),-52.5,86.0,66.5,-160,0);
    }
    ...
    private void sendMessage(Player player) {
        if (round.get(player).equals(0)) {
            player.sendMessage(ColorUtil.color("&e[NPC] 好友系统向导&f: 好友系统允许您与服务器上的好友或其他玩家连接到同一子服. &7[1/&a2&7]"));
        } else if (round.get(player).equals(1)) {
            player.sendMessage(ColorUtil.color("&e[NPC] 好友系统向导&f: 使用 &a/friend add <名字> &f来添加一个好友. 你可以通过 &a/friend list &f查看你的好友列表. &7[&a2/&a2&7]"));
            round.remove(player);
