...
public class PitNPC extends AbstractNPC {
    ...
    public List<String> getNpcDisplayName(Player player) {
        ...
        lines.add("&c先行测试开启！");
        lines.add("&e❖ &b天坑乱斗");
        lines.add(PlaceholderAPI.setPlaceholders(player, "&7当前有 &b" + Main.getInstance().getConfig().getString("total.ThePit") + " &7名玩家正在游玩"));
        return lines;
    }

    @Override
    public Location getNpcSpawnLocation() {
        return new Location(Bukkit.getWorld("world"),151.5,123.0,225.5,0,0);
    }
