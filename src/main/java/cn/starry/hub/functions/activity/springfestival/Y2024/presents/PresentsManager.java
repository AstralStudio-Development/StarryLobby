...
public class PresentsManager {
    ...
    public static void unlockPresents(UUID uuid, AbstractPresents presents) {
        if (presents == null) {
            Bukkit.getPlayer(uuid).sendMessage(ColorUtil.color("&c不存在此礼物,请联系管理员修复！"));
            return;
        }
        if (!Main.getInstance().getData().getPresentsData(uuid,"presents").contains(presents.getInternalName())) {
            ...
            Main.getInstance().getData().updatePresentsData(uuid,"presents",presents.getInternalName());
            Main.getInstance().getData().updatePresentsPoints(uuid,"points",1);
            int typePresents = 0;
            ...
            for (AbstractPresents present : Main.getInstance().getPresentsFactory().getPresents()) {
                ...
                if (Main.getInstance().getData().getPresentsData(uuid, "presents").contains(present.getInternalName())) {
                    if (present.getType().equals(presents.getType())) {
                        unlockedPresents++;
                    }
                }
            ...
            Bukkit.getPlayer(uuid).sendMessage(ColorUtil.color("&f你&b找到了&f一个大厅礼物！ &7(&b" + unlockedPresents + "&8/&b" + typePresents + "&7)"));
            ...
            Bukkit.getPlayer(uuid).sendMessage(ColorUtil.color("&c你已经领取了该礼物了！"));
        }
    }

    public static AbstractPresents getAchievement(String name) {
        for (AbstractPresents achievement : Main.getInstance().getPresentsFactory().getPresents()) {
