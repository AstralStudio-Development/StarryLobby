...
public class RebootRunnable extends BukkitRunnable {
        ...
        if (currentTask != null) {
            ...
            if (currentTask.endTime <= System.currentTimeMillis() + 5 * 1000) {
                for (Player player : Bukkit.getOnlinePlayers()) {
                    player.sendMessage(ColorUtil.color("&4&l注意! &7此大厅即将重启."));
                }
            }
        }
    }
    ...
    public void addRebootTask(RebootTask task) {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            player.sendMessage(ColorUtil.color("&4&l注意! &7一个计划中的重启即将在 &c" + time + "&7 后执行&8[#" + task.reason + "]"));
            TitleUtil.sendTitle(player, ColorUtil.color("&c服务器即将在 &b" + time + " &c后重启"), "", 20, 100, 20);
        }
    }
