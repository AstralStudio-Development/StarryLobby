...
public class CommodityButtons {
    ...
    public ItemStack PointsItem(int amount,Material material) {
        ...
        item = new ItemBuilder(material).name(ColorUtil.color(" &f" + amount + "点券 &7(" + real + "CNY)")).lore(lores).build();
    ...
    public ItemStack MysteryBoxItem(int amount, int level, int price) {
        ...
        item = new ItemBuilder(Material.ENDER_CHEST).amount(amount).name(ColorUtil.color(" &f神秘箱 &7(" + amount + ")")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack HornItem(int amount,int price) {
        ...
        item = new ItemBuilder(Material.HOPPER).amount(amount).name(ColorUtil.color(" &f喇叭 &7(" + amount + ")")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
