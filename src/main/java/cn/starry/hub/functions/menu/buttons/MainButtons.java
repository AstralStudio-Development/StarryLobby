...
public class MainButtons {
    ...
    public ItemStack PlayerProfileButton(Player player) {
        ...
        lores.add(PlaceholderAPI.setPlaceholders(player, StarryLobby.getInstance().rewards ? "&7YumeGames等级: &6%alonsolevels_level%" : "&7YumeGames等级: &8已禁用"));
        lores.add(PlaceholderAPI.setPlaceholders(player, "&7成就点数: &e%starrylobby_points%"));
        item = new ItemBuilder(Material.LEGACY_SKULL_ITEM).durability(3).name(CC.translate(LuckPermsUtil.getPlayerRank(player.getUniqueId()) + player.getName())).setModernSkullOwner(player).lore(lores).build();
    ...
    public ItemStack RewardsButton(Player player) {
        ...
        item = new ItemBuilder(Material.LEGACY_BREWING_STAND_ITEM).name(CC.translate("&aYumeGames等级")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack ShopButton(Player player) {
        ...
        item = new ItemBuilder(Material.NAME_TAG).name(CC.translate(" &f货摊")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack NewsButton(Player player) {
        ...
        item = new ItemBuilder(Material.ITEM_FRAME).name(CC.translate(" &f公告")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack GlassButton(Player player,int durability) {
        item = new ItemBuilder(Material.LEGACY_STAINED_GLASS_PANE).name(CC.translate(" ")).durability(durability).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
