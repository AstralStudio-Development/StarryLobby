...
public class DeliveryButtons {
    ...
    public ItemStack DisableButton(Player player) {
        ...
        item = new ItemBuilder(Material.BARRIER).name(CC.translate(" &c哦不...")).lore(lores).build();
    ...
    public ItemStack PublicizeVideoButton(Player player) {
        ...
        item = new ItemBuilder(Material.LEGACY_RECORD_12).name(CC.translate("&bFairylands宣传片")).lore(lores).build();
        meta = item.getItemMeta();
    ...
    public ItemStack CodeItem(Player player) {
        ...
        item = new ItemBuilder(Material.LEGACY_SIGN).name(CC.translate(" &a使用兑换码")).lore(lores).build();
        meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        return item;
    ...
    public ItemStack NewPlayerItem(Player player) {
        ...
        item = new ItemBuilder(Material.ENCHANTMENT_TABLE).name(CC.translate(" &a欢迎礼包")).lore(lores).build();
        meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        return item;
    ...
    public ItemStack getVIPItem(Player player) {
        ...
        item = new ItemBuilder(Material.ENDER_CHEST).name(CC.translate((player.hasPermission(permissionRequire) ? " &a" : " &c") + "神秘箱礼包")).lore(lores).build();
        meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        return item;
    ...
    public ItemStack getDefaultItem(Player player) {
        ...
        lores.add(" ");
        lores.add(" &e点击领取 ");
        item = new ItemBuilder(Material.ENDER_CHEST).name(CC.translate(" &a神秘箱礼包")).lore(lores).build();
        meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        return item;
    ...
    public ItemStack getDoneBoxItem(Player player) {
        ...
        lores.add(" ");
        lores.add(" &c下次在来吧 :) ");
        item = new ItemBuilder(Material.MINECART).name(CC.translate(" &c神秘箱礼包")).lore(lores).build();
        meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        return item;
    ...
    public ItemStack getDailyItem(Player player) {
        ...
        item = new ItemBuilder(isReceive ? Material.MINECART : Material.STORAGE_MINECART).name(CC.translate((isReceive ? " &c" : " &a") + "每日奖励")).lore(lores).build();
        meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        return item;
    ...
    public ItemStack getDoneDailyItem(Player player) {
        ...
        item = new ItemBuilder(Material.MINECART).name(CC.translate(" &c每日奖励")).lore(lores).build();
        meta = item.getItemMeta();
        meta.addItemFlags(ItemFlag.HIDE_ENCHANTS);
        item.setItemMeta(meta);
        return item;
