...
import cn.starry.social.bukkit.expand.guild.AllData;
...
public class SocialButtons {
    ...
    public ItemStack noFriends() {
        ...
        item = new ItemBuilder(Material.GLASS_BOTTLE).name(ColorUtil.color(" &c你还木有好友(⌯꒪꒫꒪)੭")).lore(lores).build();
    ...
    public ItemStack friendSkullItem(String name) {
        ...
        if (Bukkit.getServer().getOnlinePlayers().contains(Bukkit.getPlayer(name))) {
            ...
            item = new ItemBuilder(Material.SKULL_ITEM).durability(3).name(ColorUtil.color(LuckPermsUtil.getPlayerRank(uuid) + name)).setNewSkullOwner(player).lore(lores).build();
        } else {
            UUID uuid = UUID.fromString(Main.getInstance().getData().getPlayerData(name,"uuid"));
            item = new ItemBuilder(Material.SKULL_ITEM).durability(3).name(ColorUtil.color(LuckPermsUtil.getPlayerRankByCache(uuid)+ name)).lore(lores).build();
    ...
    public ItemStack friendAddItem() {
        ...
        item = new ItemBuilder(Material.BOOK_AND_QUILL).name(ColorUtil.color("&a添加好友")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack friendRemoveItem() {
        ...
        item = new ItemBuilder(Material.BARRIER).name(ColorUtil.color("&c删除好友")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack noParty() {
        ...
        item = new ItemBuilder(Material.STAINED_CLAY).durability(2).name(ColorUtil.color("&a创建一个组队")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack inviteParty() {
        ...
        item = new ItemBuilder(Material.BOOK_AND_QUILL).name(ColorUtil.color("&a邀请玩家")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack removeParty() {
        ...
        item = new ItemBuilder(Material.BARRIER).name(ColorUtil.color("&a删除玩家")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack warpParty() {
        ...
        item = new ItemBuilder(Material.NETHER_BRICK_ITEM).name(ColorUtil.color("&a传送组队")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack disbandParty() {
        ...
        item = new ItemBuilder(Material.TNT).name(ColorUtil.color("&a解散组队")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack clearRecentPlayers() {
        ...
        item = new ItemBuilder(Material.BARRIER).name(ColorUtil.color("&a清除近期活跃玩家")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
    ...
    public ItemStack noRecentPlayers() {
        ...
        item = new ItemBuilder(Material.GLASS_BOTTLE).name(ColorUtil.color("&c未发现近期活跃玩家")).lore(lores).build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
