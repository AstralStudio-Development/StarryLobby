...
import cn.starry.hub.api.enums.ActivityType;
...
public class ActivityButtons {
    ...
    public ItemStack LinkItem(Player player) {
        ...
        item = new ItemBuilder(Material.BOOK_AND_QUILL).name(ColorUtil.color(" &c活动链接")).lore(lores).shiny().build();
    ...
    public ItemStack ActivityStoreItem(Player player, ActivityType type) {
        ...
        item = new ItemBuilder(Material.EMERALD).name(ColorUtil.color(" &a活动商店")).lore(lores).shiny().build();
        meta = item.getItemMeta();
        item.setItemMeta(meta);
        return item;
    }
