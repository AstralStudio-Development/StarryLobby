...
public class RideCommand extends Command {
    ...
    public boolean execute(final CommandSender commandSender, final String s, final String[] strings) {
            if (commandSender instanceof Player) {
                ...
                if (!commandSender.hasPermission(Permission.RANK.getNode())) {
                    commandSender.sendMessage(CC.translate("&c你没有权限来使用这个命令"));
                    return true;
                }
                if (strings.length != 1) {
                    player.sendMessage(CC.translate("&c用法: /ride <玩家>"));
                    return true;
                }
                Player target = player.getServer().getPlayer(strings[0]);
                if (target == null) {
                    player.sendMessage(CC.translate("&c找不到指定的玩家或该玩家不在线！"));
                    return true;
                }
                if (target == player) {
                    player.sendMessage(CC.translate("&c你不能骑行你自己！"));
                    return true;
                }
                if (target.getDisplayName().equalsIgnoreCase("Stalyer")) {
                    player.sendMessage(CC.translate("&c你不能骑行插件作者！"));
                    return true;
                }
