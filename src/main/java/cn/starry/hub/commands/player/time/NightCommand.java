...
public class NightCommand extends Command {
    ...
    public boolean execute(CommandSender commandSender, String s, String[] strings) {
        ...
        if (!Objects.equals(StarryLobby.getPlugin(StarryLobby.class).getConfig().getString("type"), "Login")) {
            if (!Core.getInstance().getMongoDB().getPlayerData(player.getUniqueId(),"settings_lobby_time").equals("NIGHT")) {
                player.setPlayerTime(18000L, false);
                Core.getInstance().getMongoDB().updatePlayerData(player.getUniqueId(),"settings_lobby_time", "NIGHT");
                player.sendMessage(CC.translate("&f已将你的时间设置为 &b夜晚"));
            } else {
                player.sendMessage(CC.translate("&c当前时间已为夜晚!"));
            }
        } else {
