    ...
    public static ItemStack getItem(Player player,int id) {
        UUID uuid = player.getUniqueId();
            ...
            case ENGLISH:
                ...
                ItemHandler.PROFILE = new ItemBuilder(Material.LEGACY_SKULL_ITEM).durability(3).name("&fMy Profile &7(Right Click)").lore("&7Right-Click to Change your existing suffix,Settings","&7and more content").setModernSkullOwner(player).build();
            ...
            case JAPANESE:
                ...
                ItemHandler.PROFILE = new ItemBuilder(Material.LEGACY_SKULL_ITEM).durability(3).name("&fプロフィール &7(右クリック)").lore("&7Right-Click to Change your existing suffix,Settings","&7and more content").setModernSkullOwner(player).build();
             ...
             */
        ItemHandler.SELECTOR = new ItemBuilder(Material.PAPER).name("&a游戏菜单 &7(右键打开)").lore("&7右键打开游戏菜单").build();
        ItemHandler.STORE = new ItemBuilder(Material.NAME_TAG).name("&f货摊 &7(右键打开)").lore("&7右键打开货摊菜单", "&7可用这个购买服务器物品").build();
        ItemHandler.PROFILE = new ItemBuilder(Material.PLAYER_HEAD).name("&a个人档案 &7(右键打开)").lore("&7右键更改你已有的后缀,设置","&7以及更多内容").durability(3).setModernSkullOwner(player).build();
        ItemHandler.PROTOTYPE = new ItemBuilder(Material.GLOWSTONE_DUST).name("&a人气星尘 &7(右键打开)").lore("&7右键打开投票菜单","&7选择你希望正式上线的游戏").build();
        ItemHandler.GADGETSMENU = new ItemBuilder(Material.CHEST).name("&a收藏品 &7(右键点击)").lore("&7右键打开收藏品菜单").build();
        ItemHandler.SETTINGS = new ItemBuilder(Material.FEATHER).name("&a设置 &7(右键打开)").lore("&7右键打开设置菜单", "&7可用这个自定义你的视觉").build();
