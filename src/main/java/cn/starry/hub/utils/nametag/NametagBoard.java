    ...
    private void updateHealthBelow(Player player, Scoreboard scoreboard) {
        if (this.handler.getAdapter().showHealthBelowName(player)) {
            if (scoreboard.getObjective(DisplaySlot.BELOW_NAME) == null) {
                Objective objective = scoreboard.registerNewObjective("showping", "ping");
                objective.setDisplayName(ChatColor.WHITE + StringEscapeUtils.unescapeJava("ms ") + ChatColor.GRAY + StringEscapeUtils.unescapeJava("延迟"));
                objective.setDisplaySlot(DisplaySlot.BELOW_NAME);
                // Ensures that 0 isn't displayed if they haven't lost health.
                for (Player loopPlayer : Bukkit.getOnlinePlayers()) {
                    objective.getScore(loopPlayer).setScore((int) Math.floor(((CraftPlayer)loopPlayer).getHandle().ping));
                }
            }
        } else {
            if (scoreboard.getObjective(DisplaySlot.BELOW_NAME) != null) {
